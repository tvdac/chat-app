...
module.exports = {
    ...
    validateSignupForm: (request, response) => {
        if (request.body.name == '') {
            ...
            return AuthService.sendAuthError(response, 'Signup Failed!', "You must provide an email address to sign up", { email: request.body.email });
        }
        return true;
    },
    ...
    registerUser: async (data, response) => {
        try {
            ...
            let newUser = await User.create({ name, email, avatar }).fetch();
    ...
    login: async (request, response) => {
        try {
            ...
            if (user) { // Login Passed
                ...
                return AuthService.sendAuthError(response, 'Login Failed!', "The email provided is not registered", { email: request.body.email, name: request.body.name });
            }
        } catch (err) {
